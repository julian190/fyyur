...
@app.route('/venues')
def venues():
  # TODODone: replace with real venues data.->Done
  ...
  for x,y in locations:
    ndata= {
      ...
      'state': y,
    'venues':[]}
    qury = Venue.query.filter_by(city = x,state=y)
    count= Venue.query.filter_by(city = x,state=y).count()

    for entry in qury:
      ndata['venues'].append({'id':entry.id,'name':entry.name,'num_upcoming_shows':count})
    finaldata.append(ndata)
    print(ndata)
    print(finaldata)
...
def search_venues():
  ...
  res = {'count':xx,"data":[]}
  for entry in venue:
    count = Venue.query.filter_by(name=entry.name).count()
    res['data'].append({"id":entry.id,"name":entry.name,"num_upcoming_shows":count})
  print(res)
  response={
    "count": 1,
    "data": [{
...
def show_venue(venue_id):
  ...
  return render_template('pages/show_venue.html', venue=newdate)
...
def artists():
  ...
  return render_template('pages/artists.html', artists=new)
...
def search_artists():
  ...
  res = {'count': xx, "data": []}
  for entry in artist:
    count = Artist.query.filter_by(name=entry.name).count()
    res['data'].append({"id": entry.id, "name": entry.name, "num_upcoming_shows": count})
  print(res)
  response={
    "count": 1,
...
def show_artist(artist_id):
  # shows the venue page with the given venue_id
  # TODODone: replace with real venue data from the venues table, using venue_id
  venu = Venue.query.get(artist_id)
  newdate = {
  ...
  return render_template('pages/show_artist.html', artist=newdate)
...
def edit_artist(artist_id):
  ...
  artest = Artist.query.get(id= artist_id)
  newartist = {
    "id": artest.id,
  "name" :artest.name,
  "city" : artest.city,
  "state" : artest.state,
  "phone" : artest.phone,
  "genres" : artest.genres,
  "image_link" : artest.image_link,
  'facebook_link' : artest.facebook_link,
  }
...
def create_artist_submission():
  ...
  address = request.form.get('address')
  phone = request.form.get('phone')
  artest = Artist(name=name,city=city,state=state,address=address,phone=phone)
  error = False
  try:
    db.session.add(artest)
    db.session.commit()
  except:
    db.session.rollback()
    error = True
...
def create_show_submission():
  ...
  venuecount= Venue.query.filter_by(id=venueid).count()
  artistcount= Artist.query.filter_by(id=artistid).count()
  if venuecount <1 or artistcount<1 : error = True
  if(error ==False):
    try:
      show = Show(artist_id=artistid,venue_id=venueid)
      db.session.add(show)
      db.session.commit()
    except:
      db.session.rollback()
  ...
  # TODODone: on unsuccessful db insert, flash an error instead.
  if(error == True):flash('An error occurred. Show could not be listed.')
  # see: http://flask.pocoo.org/docs/1.0/patterns/flashing/
  return render_template('pages/home.html')
